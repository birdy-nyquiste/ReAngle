...
sequenceDiagram
    ...
    C->>F: GET /api/v1/... (Authorization: Bearer <token>)
...
| 方法 | 路径                                      | 认证               | 说明                          |
| ---- | ----------------------------------------- | ------------------ | ----------------------------- |
| POST | `/api/v1/payment/create-checkout-session` | ✅                 | 创建 Checkout 或重定向 Portal |
| POST | `/api/v1/payment/create-portal-session`   | ✅                 | 创建 Stripe 管理门户          |
| POST | `/api/v1/payment/webhook`                 | ❌ Stripe 签名验证 | Webhook 端点                  |
| GET  | `/api/v1/payment/usage`                   | ✅                 | 获取用量与订阅信息            |
...
sequenceDiagram
    ...
    U->>FE: 提交洗稿请求
    FE->>BE: POST /api/v1/rewrite (Authorization: Bearer <access_token>)
...
| `app/routers/payment.py`         | 支付相关 API 路由 + Webhook 端点                  |
| `app/routers/rewrite.py`         | 核心洗稿路由（使用 `check_usage_limit` 鉴权）     |
