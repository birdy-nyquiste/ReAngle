...
import { FolderInput, ChevronRight, Type, FileText, Link as LinkIcon, Youtube, Trash2, Loader2 } from "lucide-react"
                            ...
                            {inputItems.map(item => (
                                <div key={item.id} className="flex items-center justify-between p-2 rounded-lg bg-black/20 border border-white/5 group transition-colors hover:border-white/20">
                                    <div className="truncate flex-1 pr-2">
                                        <div className="text-xs truncate font-medium text-neutral-300">{item.meta?.title}</div>
                                        <div className="text-[9px] text-neutral-500 truncate">{item.meta?.detail}</div>
                                    </div>
                                    <button
                                        className="opacity-0 group-hover:opacity-100 text-neutral-500 hover:text-red-400 cursor-pointer p-1 transition-all"
